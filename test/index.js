...
describe('RpcClient', function() {
  ...
  it('accept many values for bool', function(done) {
    ...
    async.eachSeries([true, 'true', 1, '1', 'True'], function(i, next) {
      client.importAddress('RS9L1Cb4jhNbudBe9QeksrKs2YK3v9FYTA', '', i, function(error, parsedBuf) {
        should.not.exist(error);
        should.exist(parsedBuf);
  ...
  it('should process object arguments', function(done) {
    ...
    var obj = {'RS9L1Cb4jhNbudBe9QeksrKs2YK3v9FYTA': 1};
    async.eachSeries([obj, JSON.stringify(obj)], function(i, next) {
      client.sendMany('account', i, function(error, parsedBuf) {
        should.not.exist(error);
        should.exist(parsedBuf);
        parsedBuf.params[1].should.have.property('RS9L1Cb4jhNbudBe9QeksrKs2YK3v9FYTA', 1);
        next();
      });
    }, function(err) {
      requestStub.restore();
  ...
  it('should handle connection rejected 401 unauthorized', function(done) {
    ...
    client.getBalance('RS9L1Cb4jhNbudBe9QeksrKs2YK3v9FYTA', 6, function(error, parsedBuf) {
      requestStub.restore();
      should.exist(error);
  ...
  it('should callback with error if invalid json', function(done) {
    ...
    client.getDifficulty(function(error, parsedBuf) {
      requestStub.restore();
      should.exist(error);
      error.message.should.equal('Ravencoin JSON-RPC: Error Parsing JSON: Unexpected token o');
      done();
    });
  ...
  it('should callback with error if blank response', function(done) {
    ...
    client.getDifficulty(function(error, parsedBuf) {
      requestStub.restore();
      should.exist(error);
      error.message.should.equal('Ravencoin JSON-RPC: Error Parsing JSON: Unexpected end of input');
      done();
    });
